...
export const metadata: Metadata = {
  ...
  description: `Contact ${config.businessName}. Free estimates. ${config.isEmergencyService ? "24/7 emergency service." : "Same-day service available."}`,
};
      ...
      <section className="bg-slate-900 text-white py-14 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <h1 className="text-4xl md:text-5xl font-black mb-3">
            {config.isEmergencyService ? "Need help now?" : "Get in touch"}
          </h1>
          <p className="text-slate-300 text-lg">
            {config.isEmergencyService
              ? "Call us or fill out the form — we respond immediately."
              : "Call us or request a free estimate. Same-day response."}
          </p>
        </div>
      </section>
          ...
          <div>
            ...
            <div className="space-y-5">
              <div>
                ...
                <a href={`tel:${phoneDigits}`} className="text-2xl font-black text-orange-500 hover:text-orange-600">
                  {config.phone}
                </a>
                {config.isEmergencyService && <div className="text-sm text-slate-500 mt-1">Available 24/7</div>}
              </div>
              {config.email && (
                <div>
              ...
              <div>
                ...
                <div className="text-slate-700">
                  {config.isEmergencyService ? "24 hours a day, 7 days a week" : "Monday–Saturday · 7am–7pm"}
                </div>
              </div>
            </div>
