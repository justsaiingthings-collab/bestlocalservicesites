...
import { cleaningConfig as config } from "@bestlocal/templates";
import { CTASection } from "@bestlocal/ui";

interface Props { params: { location: string; service: string } }

export function generateStaticParams() {
  return config.locations.flatMap((loc: any) =>
    config.services.map((svc: any) => ({ location: loc.slug, service: svc.slug }))
  );
}

export function generateMetadata({ params }: Props): Metadata {
  const loc = config.locations.find((l: any) => l.slug === params.location);
  const svc = config.services.find((s: any) => s.slug === params.service);
  if (!loc || !svc) return {};
  return {
    ...
    <>
      <section className="relative min-h-[440px] flex items-center bg-slate-950 overflow-hidden">
        ...
        <div className="absolute inset-0" aria-hidden="true"
          style={{ background: "linear-gradient(to right, rgba(0,0,0,0.82) 0%, rgba(0,0,0,0.55) 60%, rgba(0,0,0,0.25) 100%)" }} />
          ...
          <div className="max-w-6xl mx-auto max-w-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Link href="/contact"
                ...
                📋 {config.isEmergencyService ? "Emergency Quote" : "Free Estimate"}
              </Link>
            </div>
          </div>
        </div>
      ...
      {svc.problems && svc.problems.length > 0 && (
        <section className="bg-white py-12 px-4 border-b">
          <div className="max-w-6xl mx-auto">
            <h2 className="text-2xl font-black text-slate-900 mb-6">What we help with in {loc.city}</h2>
      ...
      <section className="bg-slate-50 py-12 px-4">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-2xl font-black text-slate-900 mb-6">More services in {loc.city}</h2>
          <div className="flex flex-wrap gap-3">
            {config.services.filter((s: any) => s.slug !== svc.slug).map((s: any) => (
      ...
      <section className="bg-white py-12 px-4 border-t">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-wrap gap-3">
            {config.locations.filter((l: any) => l.slug !== loc.slug).map((l: any) => (
